...
from .block import DFL, SAVPE, BNContrastiveHead, ContrastiveHead, Proto, Residual, SwiGLUFFN, AnglePriorModule
from .conv import Conv, DWConv
from .transformer import MLP, DeformableTransformerDecoder, DeformableTransformerDecoderLayer
from .utils import bias_init_with_prob, linear_init

__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment", "AngleAwareDetect"
...
# ultralytics/nn/modules/head.py （追加）
...
# class AnglePriorModule(nn.Module):
#     """
#     基于笔画角度先验调整置信度的模块。
#     假设有效笔画角度集中在 0° 和 90°（即水平/垂直）。
#     使用双峰正态分布建模：N(0, σ²) + N(90, σ²)
#     """
# 
#     def __init__(self, sigma=15.0):  # sigma in degrees
#         super().__init__()
#         self.sigma = nn.Parameter(torch.tensor(sigma / 180 * math.pi))  # 转为弧度，可学习
#         self.w = nn.Parameter(torch.tensor(0.7))  # 融合权重 w，可学习
# 
#     def estimate_dominant_angle(self, x):
#         """
#         从特征图 x (B, C, H, W) 估计每个位置的主方向（用 Sobel 近似）。
#         返回角度图 theta ∈ [-π/2, π/2]
#         """
#         B, C, H, W = x.shape
# 
...
# 
#         #关于x、y方向的梯度图
#         Gx = F.conv2d(F.pad(gray, (1, 1, 1, 1), mode='replicate'), sobel_x, padding=0)
#         Gy = F.conv2d(F.pad(gray, (1, 1, 1, 1), mode='replicate'), sobel_y, padding=0)
# 
#         theta = torch.atan2(Gy, Gx)  # (-180, 180]
#         # 转换到 [0, 90]（因为笔画无方向性，90 和 -90 等价）
#         theta = torch.abs(theta)
#         theta = torch.where(theta > math.pi / 2, math.pi - theta, theta)  # 映射到 [0, 90]
#         return theta  # (B,1,H,W)
# 
#     def forward(self, features, raw_cls_scores):
#         """
#         Args:
#             features: (B, C, H, W) — 来自 neck 的特征图
#             raw_cls_scores: (B, nc, H, W) — 原始分类 logits（未 sigmoid）
#         Returns:
#             adjusted_conf: (B, 1, H, W) — 调整后的置信度
#         """
#         B, nc, H, W = raw_cls_scores.shape
# 
#         # Step 1: 估计主方向
#         theta = self.estimate_dominant_angle(features)  # (B,1,H,W)
#         theta_deg = theta * 180 / math.pi  # 转为角度 [0, 90]
# 
#         # Step 2: 双峰正态分布概率密度（0 和 90）
#         sigma_deg = torch.clamp(self.sigma * 180 / math.pi, min=1.0, max=45.0)
#         prob_0 = torch.exp(-0.5 * (theta_deg / sigma_deg) ** 2)
#         prob_90 = torch.exp(-0.5 * ((90 - theta_deg) / sigma_deg) ** 2)
#         angle_score = (prob_0 + prob_90) / 2  # 归一化到 [0,1] 附近
# 
#         # Step 3: 原始置信度（假设 obj 分支存在，或用 cls max）
#         orig_conf = torch.sigmoid(raw_cls_scores).max(dim=1, keepdim=True)[0]  # (B,1,H,W)
# 
#         # Step 4: 融合
#         w = torch.sigmoid(self.w)  # 限制 w ∈ (0,1)
#         adjusted_conf = w * orig_conf + (1 - w) * angle_score
# 
#         return adjusted_conf

#修改 DecoupledDetect 或原版 Detect，加入此模块：
class AngleAwareDetect(Detect):
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        super().__init__(nc, ch)
        self.angle_modules = nn.ModuleList(AnglePriorModule() for _ in ch)

    def forward(self, x: List[torch.Tensor]) -> Union[List[torch.Tensor], Tuple]:
        if self.end2end:
            return self.forward_end2end(x)

        # === 关键：无论训练还是推理，都要先通过 cv2/cv3 ===
        box_outputs = []
        cls_outputs = []
        feats = x  # 保存原始特征图，用于角度计算

        for i in range(self.nl):
            box_out = self.cv2[i](x[i])   # (B, 64, H, W)
            cls_out = self.cv3[i](x[i])   # (B, nc, H, W)
            box_outputs.append(box_out)
            cls_outputs.append(cls_out)

        if self.training or not self.export:
            # 训练：返回拼接后的 logits
            return [torch.cat((box, cls), 1) for box, cls in zip(box_outputs, cls_outputs)]

        # 推理：手动执行 _inference 逻辑，但保留特征图
        # Step 1: 拼接并 reshape
        shape = box_outputs[0].shape  # BCHW
        box_cat = torch.cat([b.view(shape[0], -1, b.shape[-2] * b.shape[-1]) for b in box_outputs], -1)  # (B, 64, N)
        cls_cat = torch.cat([c.view(shape[0], -1, c.shape[-2] * c.shape[-1]) for c in cls_outputs], -1)  # (B, nc, N)
        x_cat = torch.cat((box_cat, cls_cat), 1)  # (B, 144, N)

        # Step 2: 更新 anchors/strides if needed
        if self.format != "imx" and (self.dynamic or self.shape != shape):
            dummy_x = [torch.empty_like(b[:, :1]) for b in box_outputs]  # 伪造 BCHW 输入
            self.anchors, self.strides = (t.transpose(0, 1) for t in make_anchors(dummy_x, self.stride, 0.5))
            self.shape = shape

        # Step 3: 解码 bbox
        box = x_cat[:, : self.reg_max * 4]
        cls = x_cat[:, self.reg_max * 4 :]
        dbox = self.decode_bboxes(self.dfl(box), self.anchors.unsqueeze(0)) * self.strides  # (B, 4, N)

        # Step 4: 计算角度分数
        angle_scores_list = []
        for i, feat in enumerate(feats):
            angle_score = self.angle_modules[i](feat, cls_outputs[i])  # (B, 1, H, W)
            angle_scores_list.append(angle_score.view(angle_score.shape[0], 1, -1))  # (B, 1, N_i)
        angle_scores = torch.cat(angle_scores_list, dim=2)  # (B, 1, N)

        # Step 5: 融合置信度
        cls_sigmoid = cls.sigmoid()
        orig_conf = cls_sigmoid.max(dim=1, keepdim=True)[0]  # (B, 1, N)
        w = torch.sigmoid(self.angle_modules[0].w)
        adjusted_conf = w * orig_conf + (1 - w) * angle_scores  # (B, 1, N)

        # Step 6: 构造最终输出 [xyxy, conf, cls]
        _, cls_idx = cls_sigmoid.max(dim=1)  # (B, N)
        output = torch.cat([
            dbox.transpose(1, 2),           # (B, N, 4)
            adjusted_conf.transpose(1, 2),  # (B, N, 1)
            cls_idx.float().unsqueeze(-1)   # (B, N, 1)
        ], dim=-1)  # (B, N, 6)

        if not self.export:
            output = self.postprocess_custom(output, self.max_det)
        return output if self.export else (output, x)

    @staticmethod
    def postprocess_custom(preds: torch.Tensor, max_det: int) -> torch.Tensor:
        batch_size, n_boxes, _ = preds.shape
        conf = preds[:, :, 4]
        topk_vals, topk_idx = torch.topk(conf, min(max_det, n_boxes), dim=1)
        batch_idx = torch.arange(batch_size, device=conf.device).unsqueeze(1)
        return preds[batch_idx, topk_idx]
# class AngleAwareDetect(nn.Module):
...
#
#         # 原始 head 结构（以解耦头为例）
#         self.cls_convs = nn.ModuleList(nn.Sequential(Conv(x, x, 3, 1), Conv(x, x, 3, 1)) for x in ch)
#         self.cls_preds = nn.ModuleList(nn.Conv2d(x, nc, 1) for x in ch)
#         self.reg_convs = nn.ModuleList(nn.Sequential(Conv(x, x, 3, 1), Conv(x, x, 3, 1)) for x in ch)
#         self.reg_preds = nn.ModuleList(nn.Conv2d(x, 4 * self.reg_max, 1) for x in ch)
#
#         # 新增：角度先验模块（每个尺度一个）
#         self.angle_modules = nn.ModuleList(AnglePriorModule() for _ in ch)
#
...
#
#             cls_raw = self.cls_preds[i](cls_feat)  # (B, nc, H, W)
#             reg_raw = self.reg_preds[i](reg_feat)  # (B, 4*reg_max, H, W)
#
#             # 关键：用 reg_feat 或 cls_feat 作为方向估计输入
#             angle_conf = self.angle_modules[i](reg_feat, cls_raw)  # (B,1,H,W)
#
#             # 将调整后的置信度“注入”到分类输出中
#             # 方法：将 angle_conf 作为门控，调制分类 logits
#             cls_adjusted = cls_raw * angle_conf  # 元素乘（可选其他融合方式）
#
#             out = torch.cat([reg_raw, cls_adjusted], dim=1)
#             outs.append(out)
#
#         if self.training:
#             return outs
#
#         # 推理时：angle_conf 已隐含在 cls 中
#         pred_distri, pred_scores = torch.cat(outs, dim=1).split((self.reg_max * 4, self.nc), dim=1)
#         pred_scores = pred_scores.sigmoid()
#         anchors, strides = make_anchors(outs, self.stride, 0.5)
#         pred_bboxes = dist2bbox(pred_distri, anchors.unsqueeze(0), xywh=False, dim=1) * strides
#         return torch.cat([pred_bboxes, pred_scores], dim=1)

#检验
# model = YOLO("yolov8-angle.yaml")
# print(model.model[-1].angle_modules[0].w)      # 应接近 0～1
# print(model.model[-1].angle_modules[0].sigma) # 应接近 0.2～0.8 (弧度)