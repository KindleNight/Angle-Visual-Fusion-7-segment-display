...
from .block import DFL, SAVPE, BNContrastiveHead, ContrastiveHead, Proto, Residual, SwiGLUFFN, AnglePriorModule, C2f
from .conv import Conv, DWConv
from .transformer import MLP, DeformableTransformerDecoder, DeformableTransformerDecoderLayer
from .utils import bias_init_with_prob, linear_init

__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment", "AngleAwareDetect", "ImageAngle", "Data_fusion"
...
class Detect(nn.Module):
    ...
    def _inference(self, x: List[torch.Tensor]) -> torch.Tensor:
        ...
        x_cat = torch.cat([xi.view(shape[0], self.no, -1) for xi in x], 2)  #将2维图片压缩到一维
        if self.format != "imx" and (self.dynamic or self.shape != shape):
            self.anchors, self.strides = (x.transpose(0, 1) for x in make_anchors(x, self.stride, 0.5))     #动态调整strides和anchors，YOLOv8支持不同大小的图片输入
            self.shape = shape

        #将x_cat拆分成box(识别框)和cls(类别)
...
class ImageAngle(nn.Module):
    def __init__(self, num_classes, ch_list: tuple, kernel_size: int = 3):
        super().__init__()
        self.num_classes = num_classes
        padding = kernel_size // 2

        # 为每个输入尺度创建 AnglePriorModule
        self.angle_module = AnglePriorModule()

        # 为每个尺度创建可训练的卷积层
        # 输入通道数为 1（角度图），输出通道数为 num_classes
        self.conf_convs = nn.ModuleList([
            nn.Conv2d(
                in_channels=1,  # 角度图只有一个通道
        ...
        for conv in self.conf_convs:
            nn.init.kaiming_normal_(conv.weight, mode='fan_out', nonlinearity='tanh')
    ...
    def forward(self, x):
        ...
        # 1. 提取原始角度图 (基于 P2 尺度，即原图 1/4)
        angle_map_base = self.angle_module(x)

        confidences = []

        # 2. 对应 P3, P4, P5 的相对下采样倍数
        # P3(1/8) = P2 / 2
        # P4(1/16) = P2 / 4
        # P5(1/32) = P2 / 8
        relative_strides = [1, 2, 4]

        for i, s in enumerate(relative_strides):
            # 使用相对步长进行池化
            # 如果此时尺寸还是微小对不上，Detect 里的 interpolate 会处理，
            # 但这里必须尽可能接近目标尺寸
            feat = F.max_pool2d(angle_map_base, kernel_size=s, stride=s)

            # 卷积映射到类别通道
            logits = self.conf_convs[i](feat)

            # 暴力测试建议：暂时去掉 tanh，或者改用一个系数较大的激活
            # 因为 tanh 会把输出死死限制在 -1 到 1，可能被视觉分支淹没
            confidences.append(logits)

        return confidences


class AngleAwareDetect(Detect):
    def __init__(self, nc: int = 11, ch: Tuple = ()):
        valid_ch = [c for c in ch if c != -1]
        super().__init__(nc, ch[:-1])
        self.all_ch = ch
        self.angle_scale = nn.Parameter(torch.tensor(-2.0))
        self.fusion_weight = nn.Parameter(torch.full((1,), 0.1))

    def forward(self, x: List[torch.Tensor]):
        base_features = x[:-1]
        angle_logits_list = x[-1]

        # ... 前面的 Stride 检查等逻辑保持不变 ...

        outputs = []
        w = torch.sigmoid(self.angle_scale)

        # --- 策略开关 ---
        # 假设 50 Epoch 前为稳定期，50 之后为微调期
        # 这里的 getattr 是为了安全获取当前 epoch，如果拿不到则默认使用稳定模式
        current_epoch = getattr(self, 'current_epoch', 0)
        use_detach = current_epoch < 10
        # print(current_epoch)

        for i in range(self.nl):
            box_out = self.cv2[i](base_features[i])
            visual_cls = self.cv3[i](base_features[i])

            if i < len(angle_logits_list):
                angle_feat = angle_logits_list[i]
                if angle_feat.shape[-2:] != visual_cls.shape[-2:]:
                    angle_feat = F.interpolate(angle_feat, size=visual_cls.shape[-2:], mode='bilinear')

                # --- 动态梯度策略 ---
                if self.training:
                    if use_detach:
                        # 前 50 Epoch：角度分支只出数值，不回传梯度，保护主干
                        fused_cls = visual_cls + angle_feat.detach()
                    else:
                        fused_cls = visual_cls + angle_feat
                else:
                    fused_cls = visual_cls + angle_feat

            outputs.append(torch.cat((box_out, fused_cls), 1))

        if self.training:
            return outputs

        return self._inference(outputs) if self.export else (self._inference(outputs), outputs)


class Data_fusion(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        self.in_channels = sum(c1) if isinstance(c1, list) else c1

        self.out_channels = c2

        print(f"DEBUG FIX: Fusion -> In: {self.in_channels}, Out: {self.out_channels}")

        self.fusion_conv = Conv(self.in_channels, self.out_channels, k=1)
        self.c2 = self.out_channels

    def forward(self, x):
        fusion = torch.cat(x, 1)
